...
class Links extends Model {
   ...
   * as a string or is rejected with the error that occurred.
   */
  getUrlTitle(url) {
    return axios.get(url).then((response) => {
      const tag = /<title>(.*)<\/title>/;
      const match = response.data.match(tag);
   ...
   * of the insert query or is rejected with the error that occurred.
   */
  create(link) {
